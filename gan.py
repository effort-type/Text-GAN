...
class GAN2vec:
    ...
    def train(self, epochs, batch_size=128, save_interval=50):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0:
                model.save("gan2vec_model_" + str(epoch) + ".keras")
    ...
    def predict(self, num_sentences=5):  # 매개변수 추가
        noise = np.random.normal(0, 1, (num_sentences, self.latent_dim))
        gen_sentence = self.generator.predict(noise)
        test = np.squeeze(gen_sentence)
